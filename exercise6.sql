...
CREATE TABLE Email_data(id INT UNSIGNED NOT NULL AUTO_INCREMENT, email VARCHAR(40) NOT NULL, phone VARCHAR(20) NOT NULL, city VARCHAR(20) NOT NULL, PRIMARY KEY (id)); 

LOAD DATA LOCAL INFILE 'table1.csv' INTO TABLE Email_data FIELDS TERMINATED BY ',' ENCLOSED BY '"' (email, phone, city); 
...
SELECT DISTINCT city FROM Email_data;

--How many people responded from each city
SELECT DISTINCT city, COUNT(city) FROM Email_data GROUP BY city;  
